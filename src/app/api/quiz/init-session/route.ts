...
export async function POST(request: NextRequest) {
  ...
  response.cookies.set('nutriplan_session_id', data.id, {
    httpOnly: true,
    // 26/07 à noite: revertido de SameSite=None de volta pra lax. O None fez
    // parte de um lote de mudanças sob suspeita durante a sequência de
    // não-inícios; rollback pra isolar variáveis (ver memória
    // ios-quiz-abandonment-investigation).
    sameSite: 'lax',
    path: '/',
